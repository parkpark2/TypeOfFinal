package com.typeof.hajota.websocket;
...
import com.typeof.hajota.chatting.model.MessageVO;
...
public class WebsocketEchoHandler extends TextWebSocketHandler {
        ...
        */
	    ...
	    protected void handleTextMessage(WebSocketSession session, TextMessage message) 
            ...
            */
	    	...
	    	HashMap<String, Object> loginuser = (HashMap<String, Object>)map.get("loginuser");
	    	System.out.println(">>>>>> 확인용 사용자ID : " + loginuser.get("EMAIL"));
	    	// >>>>>> 확인용 사용자ID : seoyh
	            ...
	            if (messageVO.getType().equals("all")) { // 채팅할 대상이 "전체" 일 경우 
	                if (!session.getId().equals(webSocketSession.getId())) {  // 메시지를 자기자신을 뺀 나머지 모든 사용자들에게 메시지를 보냄.
	                    webSocketSession.sendMessage(
	                            new TextMessage(" [" +loginuser.get("LAST_NAME")+loginuser.get("FIRST_NAME")+ "]" + " ▶ " + messageVO.getMessage()));  
	                }
	                ...
	                if (messageVO.getTo().equals(hostName)) {
	                    webSocketSession.sendMessage(
	                            new TextMessage(
	                                    ...
	                                    + " [" +loginuser.get("LAST_NAME")+loginuser.get("FIRST_NAME")+ "]" + "▶ " + messageVO.getMessage()
	                                    + "</span>") );
	                    break;
	                }
	            }
        ...
        */
	    ...
	    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) 
	        ...
	        for (WebSocketSession webSocketSession : connectedUsers) {
	            if (!session.getId().equals(webSocketSession.getId())) { // 메시지를 자기자신을 뺀 나머지 모든 사용자들에게 메시지를 보냄.
	                webSocketSession.sendMessage(new TextMessage(session.getRemoteAddress().getHostName() +" [" +loginuser.get("LAST_NAME")+loginuser.get("FIRST_NAME")+ "]" + "님이 퇴장했습니다."));
	            }
	        }
