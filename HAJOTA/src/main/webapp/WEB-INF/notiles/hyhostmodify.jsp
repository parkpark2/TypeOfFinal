...
$(document).ready(function(){
	$("#roomsname").val("${roomInfo.NAME}");
	$("#TYPE_LODGE").val("${roomInfo.TYPE_LODGE}");
	$("#TYPE_BUILDING").val("${roomInfo.TYPE_BUILDING}");
	$("#maxpeople").val("${roomInfo.NUM_OF_PEOPLE}");
	
	$("#sample4_roadAddress").val("${roomInfo.LOCATION}");
	$("#detailAddress").val("${roomInfo.LOCATION_DETAIL}");
	$("#LAT").val("${roomInfo.LAT}");
	$("#LNG").val("${roomInfo.LNG}");
	
	$("#bathroomcount").val("${roomInfo.NUM_OF_BATHROOM}");
	$("#bedroomcount").val("${roomInfo.NUM_OF_BEDROOM}");
	$("#bedcount").val("${roomInfo.NUM_OF_BED}");
	
	$("#checkinHour").val("${roomInfo.CHECKIN_HOUR}");
	$("#checkinMin").val("${roomInfo.CHECKIN_MIN}");
	$("#checkoutHour").val("${roomInfo.CHECKOUT_HOUR}");
	$("#checkoutMin").val("${roomInfo.CHECKOUT_MIN}");
	
	$("#roomprice").val("${roomInfo.ROOM_PRICE}");
	$("#depositprice").val("${roomInfo.DEPOSIT_PRICE}");
	$("#cleanprice").val("${roomInfo.CLEAN_PRICE}");
	$("#totalprice").val("${roomInfo.TOTAL_PRICE}");
	
	if(${roomInfo.ELEVATOR == 1}) {
		$('#elevator').attr('checked', 'checked');
	}
	...
	if(${roomInfo.AIR_CONTROLLER == 1}) {
		$('#airconditioner').attr('checked', 'checked');
	}
	
	if(${roomInfo.HEATING == 1}) {
		$('#heater').attr('checked', 'checked');
	}
	
	if(${roomInfo.WASHING_MACHINE == 1}) {
		$('#washingmachin').attr('checked', 'checked');
	}
	
	if(${roomInfo.HAIR_DRYER == 1}) {
		$('#hairdry').attr('checked', 'checked');
	}
	
	if(${roomInfo.TV == 1}) {
		$('#television').attr('checked', 'checked');
	}
	
	if(${roomInfo.ESSENTIAL_ITEM == 1}) {
		$('#essentialitem').attr('checked', 'checked');
	}
	
	$("#rooms_explain").val("${roomInfo.ROOMS_EXPLAIN}");
	$("#refund_policy").val("${roomInfo.REFUND_POLICY}");
	$("#rooms_rule").val("${roomInfo.ROOMS_RULE}");
	
	$("#safety_function").val("${roomInfo.SAFETY_FUNCTION}");
	$("#local_information").val("${roomInfo.LOCAL_INFORMATION}");
	
	// 보안 문제로 값을 넣는 건 불가함....
	// $("#mainimage").val("${roomInfo.IMG_MAIN}");
...
<form id="msform" name ="msform" enctype="multipart/form-data">
  ...
  <fieldset>
	...
	<option value="1" selected="selected">집전체</option>
	...
	<option value="1" selected="selected">1 명</option>
